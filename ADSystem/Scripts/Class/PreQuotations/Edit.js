angular.module("General").controller('PreQuotationsController', function (models, $scope, $http, GUID, notify, $rootScope, $timeout) {
    ...
    InitFabricsData = function (fabrics) {

        let arrayInitFabrics = [];

        if (fabrics.length) {
            fabrics.forEach((item) => {
                arrayInitFabrics.push({
                    idFabricItem: item.idTextiles + '-' + GUID.New(),
                    idFabric: item.idTextiles,
                    fabric: item.NombreTextiles,
                    //brand: item.Marca,
                    price: item.CostoPorMts
                });

            })

            $scope.itemsFabrics = arrayInitFabrics;
        }
        return arrayInitFabrics;
    }

    $scope.LoadInformation = function (typeCustomer, customer, idOffice, idBranch, seller, prequotation) {

        $scope.idPreQuotation = prequotation.idPreCotizacion;
        ...
        if (oDetail.length) {
            ...
            oDetail.forEach((item) => {
                ...
                $scope.items.push({
                    idProduct: item.idServicio + '-' + GUID.New(),
                    idService: item.idServicio,
                    image: item.Imagen,
                    pdf: item.PDF,
                    code: "SERVICIO",
                    desc: item.Descripcion,
                    existence: item.Cantidad, /* DUDA DATOS A REGRESAR*/
                    stock: item.Cantidad,
                    ...
                    fabrics: InitFabricsData(item.fabrics),
                    measures: item.measures,
                    measureStandar: false,
                });
    ...
    $scope.AddService = () => {
        ...
        if ($scope.newService.descService.MedidasEstandar > 0) {
            if ($scope.listMeasuresStandar == undefined || $scope.listMeasuresStandar == null) {
    ...
    $scope.ResumePreQuotation = (confirm) => {
        if (confirm) {
            ...
            $http({
                method: 'POST',
                url: '../../../PreQuotations/UpdatePreQuotation',
    ...
    function generateArray() {
        ...
        return {
            "idPreCotizacion": $scope.idPreQuotation,
            "Numero": $scope.number,
