angular.module("General").controller('ReceptionsController', function ($scope, $http, $window, notify, $rootScope) {
    ...
    $scope.LoadInit = function (reception) {

        $scope.idReception = reception.idRecepcion;
        $scope.idTransfer = reception.idTransferencia;
        $scope.sellerName = reception.Usuario;
        $scope.transfernumber = reception.Transferencia;

        $scope.LoadSellers();

        $scope.number = reception.Numero;
        $scope.branch = reception.SucursalOrigen;
        $scope.branchID = reception.idSucursalOrigen;
        $scope.branchDestiny = reception.idSucursalDestino;
        $scope.comments = reception.Comentarios;
        
        angular.forEach(reception.lDetail, function (value, key) {

            $scope.items.push({
                idDetalleRecepcion: value.idDetalleRecepcion,
                idProducto: value.idProducto,
                imagen: value.Imagen,
                codigo: value.Codigo,
                desc: value.Descripcion,
                prec: value.Costo,
                enviado: value.Cantidad - value.CantidadRecibida,
                recibido: 0
            });
    ...
    $scope.SaveReception = function () {
        ...
        //Se valida que no existan cantidad mayores a las enviadas
        ...
        //Se valida que por lo menos exista un productos recibido
        angular.forEach($scope.items, function (value, key) {
            if (!withoutZero) {
                if (value.recibido > 0) {
                    withoutZero = true;
        ...
        if (keepGoing && withoutZero) {
            ...
            angular.forEach($scope.items,
                function(value, key) {
                    ...
                    lHistoryReception.push({
                        idDetalleRecepcion: value.idDetalleRecepcion,
                        Cantidad: value.recibido,
                    });
                });

            reception = {
                idRecepcion: $scope.idReception,
                Numero: $scope.number,
            ...
            $http({
                method: 'POST',
                url: '../../../Receptions/SaveUpdate',
                data: {
                    'reception': reception,
                    'lDetail': lDetail,
                    'lHistoryReception': lHistoryReception,
                ...
                if (data.success == 1) {
                    ...
                    window.location = "../../../Receptions/List";
                } else if (data.failure == 1) {
                    notify(data.oData.Error, $rootScope.error);
                } else if (data.noLogin == 1) {
                    window.location = "../../../Access/Close";
