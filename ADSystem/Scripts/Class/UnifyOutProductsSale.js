angular.module("General").controller('SaleController', function (models, ServiceSaleValidator, PaymentValidator, DetailSaleFacturaValidator, GUID, GetDiscount, GetDiscountOffice, GetDiscountSpecial, $scope, $http, $window, notify, $rootScope, FileUploader) {
            ...
            success(function (data, status, headers, config) {
                ...
                if (data.success == 1) {
                    ...
                    if (result == undefined) {
                        ...
                        idVista = $scope.items[index].idView;
                    }
    ...
    $scope.ValidateStock = function (ID, branchID, viewID) {
        ...
        if (service == false) {
            ...
            if (exist < amount) {
                ...
                //Se actualiza el inventario según el artículo y la  vista
                angular.forEach($scope.items, function (value, key) {
                    if (value.idProducto == ID && value.idSucursal == branchID && value.idView == viewID) {
                        value.cantidad = exist;
                    }
                });
                ...
                if ($scope.items[index].idView > 0) {
                    $scope.GetStockProductOutProduct(ID, $scope.items[index].idView);
                } else {
