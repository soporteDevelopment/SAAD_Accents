angular.module("General").controller('EditSalesController', function (models, ServiceSaleValidator, PaymentValidator, DetailSaleFacturaValidator, GUID, GetDiscount, GetDiscountOffice, GetDiscountSpecial, $scope, $http, $window, notify, $rootScope, FileUploader) {
    ...
    $scope.LoadInformation = function (typeCustomer, customer, idOffice, seller, sale, discount, iva) {
        ...
        $scope.branchID = (sale.idSucursal == 1) ? 2 : sale.idSucursal;
        $scope.unified = sale.idSucursal;
            ...
            success(function (data, status, headers, config) {
                ...
                if (data.success == 1) {
                    ...
                    var result = _.result(_.find($scope.items, function (chr) {
                        return chr.idProducto == data.oData.Product.idProducto && chr.idSucursal == $scope.branchID && chr.idPromocion == null
                    }), 'idProducto');
            ...
            $http({
                ...
                success(function (data, status, headers, config) {
                    ...
                    if (data.success == 1) {
                        ...
                        var branchID = (prod.idSucursal > 0) ? prod.idSucursal : $scope.branchID;

                        var result = _.result(_.find($scope.items, function (chr) {
                            return chr.idProducto == data.oData.Product.idProducto && chr.idSucursal == branchID && chr.idPromocion == null
                        }), 'idProducto');
    ...
    $scope.SavePayment = function () {
        ...
        if (($scope.resCredit > 0) || (paymentEmpty != undefined)) {
            ...
            $http({
                ...
                data: {
                    idSale: $scope.idSale,
                    idBranch: ($scope.unified == 1) ? $scope.unified : $scope.branchID,
