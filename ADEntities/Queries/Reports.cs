...
namespace ADEntities.Queries
{
    public class Reports : Base
    {
        ...
        public List<ReporteEstatusPrecotizaciones_Result> PreQoutationsReport(DateTime startDate, DateTime endDate, string preQoutationNumber,string project , int? typecustomer, int? idCustomer, int? idVendor)  
        {
            using (var context = new admDB_SAADDBEntities())
                try
                {
                    ...
                    return context.ReporteEstatusPrecotizaciones(startDate,  endDate,  preQoutationNumber,  project,typecustomer,   idCustomer,  idVendor).ToList();
                }
                catch (DbEntityValidationException ex)
                {
                    var newException = new ADEntities.Common.FormattedDbEntityValidationException(ex);
        ...
        public List<ReporteEstatusServicios_Result> ServiceReport(DateTime startDate, DateTime endDate, string preQoutationNumber, int? idService, int? idProvider)
        {
            using (var context = new admDB_SAADDBEntities())
                try
                {
                    startDate = startDate.Date + new TimeSpan(0, 0, 0);
                    endDate = endDate.Date + new TimeSpan(23, 59, 59);
                    return context.ReporteEstatusServicios(startDate, endDate, preQoutationNumber, idService, idProvider).ToList();
                }
                catch (DbEntityValidationException ex)
                {
                    var newException = new ADEntities.Common.FormattedDbEntityValidationException(ex);
